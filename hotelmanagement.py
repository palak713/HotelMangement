...
msg2 = "Welcome to our Palak Hotel.."
...
def TotalResult():
    ...
    if varme1 == 'Fried Rice':
        ...
        varme3 = (varme2 * 800)
        Cost.set(varme3)
    elif varme1 == 'Vegplate':
        varme3 = (varme2 * 800)
        Cost.set(varme3)
    elif varme1 == 'Garlicbread':
        varme3 = (varme2 * 100)
        Cost.set(varme3)
    elif varme1 == 'Burger':
        varme3 = (varme2 * 140)
        ...
        varme3 = (varme2 * 300)
        Cost.set(varme3)
    elif varme1 == 'Stuffed Paratha':
        varme3 = (varme2 * 401)
        Cost.set(varme3)
    elif varme1 == 'Salad':
        varme3 = (varme2 * 391)
        Cost.set(varme3)
    elif varme1 == 'ChikeenTikka':
        varme3 = (varme2 * 1000)
        Cost.set(varme3)
    elif varme1 == 'Panner+Naan':
        varme3 = (varme2 * 1000)
        Cost.set(varme3)
    elif varme1 == 'Spclplate':
        varme3 = (varme2 * 900)
        Cost.set(varme3)
    else:
        varme3 = (varme2 * 0.0)
    ...
    if vardi1 == 'Bottled Water':
        ...
        vardi3 = (vardi2 * 301)
        Drinks.set(vardi3)
    elif vardi1 == 'Milk Shake':
        vardi3 = (vardi2 * 251)
        Drinks.set(vardi3)
    elif vardi1 == 'Chocalateshake':
        vardi3 = (vardi2 * 301)
        Drinks.set(vardi3)
    elif vardi1 == 'Greentea':
        vardi3 = (vardi2 * 280)
        Drinks.set(vardi3)
    elif vardi1 == 'Coffee':
        vardi3 = (vardi2 * 345)
    ...
    rvip1 = Delivery / (1000 * 0.5)  # ====VIP room with Delivery cost===================

    rnormal = 5.0  # =========Cost of Normal room====================================
    rnormal1 = Delivery / (500 * 2.5)  # ======Normal room with Delivery cost============
...
# ================================Window's Partition==============================
Tops = Frame(root, width=1600, height=100, bg='orange', relief=SUNKEN)
Tops.pack(side=TOP)
...
# ===================================Main Screen==================================
txt_input = StringVar(value='>>Hotel Palak Paradise')
Display = Entry(Tops, font=('arial', 97, 'bold'), fg='white', bd=50, bg='orange', justify='right', textvariable=txt_input)
...
btn7 = Button(f2, padx=15, pady=5, bd=8, font=('arial', 30, 'bold'), text='7', command=lambda: btn(7)).grid(row=2,
                                                                                                            column=0)
btn8 = Button(f2, padx=15, pady=5, bd=8, font=('arial', 30, 'bold'), text='8', command=lambda: btn(8)).grid(row=2,
                                                                                                            column=1)
btn9 = Button(f2, padx=15, pady=5, bd=8, font=('arial', 30, 'bold'), text='9', command=lambda: btn(9)).grid(row=2,
                                                                                                            column=2)
btnC = Button(f2, padx=15, pady=5, bd=8, font=('arial', 30, 'bold'), text='C', bg='green', command=Clear).grid(row=2,
                                                                                                               column=3)

# ==================================Row 2========================================

btn4 = Button(f2, padx=15, pady=5, bd=8, font=('arial', 30, 'bold'), text='4', command=lambda: btn(4)).grid(row=3,
                                                                                                            column=0)
btn5 = Button(f2, padx=15, pady=5, bd=8, font=('arial', 30, 'bold'), text='5', command=lambda: btn(5)).grid(row=3,
                                                                                                            column=1)
btn6 = Button(f2, padx=15, pady=5, bd=8, font=('arial', 30, 'bold'), text='6', command=lambda: btn(6)).grid(row=3,
                                                                                                            column=2)
...
btn1 = Button(f2, padx=15, pady=5, bd=8, font=('arial', 30, 'bold'), text='1', command=lambda: btn(1)).grid(row=4,
                                                                                                            column=0)
btn2 = Button(f2, padx=15, pady=5, bd=8, font=('arial', 30, 'bold'), text='2', command=lambda: btn(2)).grid(row=4,
                                                                                                            column=1)
btn3 = Button(f2, padx=15, pady=5, bd=8, font=('arial', 30, 'bold'), text='3', command=lambda: btn(3)).grid(row=4,
                                                                                                            column=2)
...
btn0 = Button(f2, padx=15, pady=5, bd=8, font=('arial', 30, 'bold'), text='0', command=lambda: btn(0)).grid(row=5,
                                                                                                            column=0)
btndot = Button(f2, padx=21, pady=5, bd=8, font=('arial', 30, 'bold'), text='.', bg='orange', command=lambda: btn('.')
                ).grid(row=5, column=1)
btndivision = Button(f2, padx=20, pady=5, bd=8, font=('arial', 30, 'bold'), text='/', bg='orange',
                     command=lambda: btn('/')
                     ).grid(row=5, column=2)
btnmultiply = Button(f2, padx=19, pady=5, bd=8, font=('arial', 30, 'bold'), text='x', bg='orange',
                     command=lambda: btn('*')
                     ).grid(row=5, column=3)
...
btnopenbracket = Button(f2, padx=19, pady=2, bd=8, font=('arial', 30, 'bold'), text='(', bg='orange',
                        command=lambda: btn('(')
                        ).grid(row=6, column=2)
btnclosebracket = Button(f2, padx=23, pady=2, bd=8, font=('arial', 30, 'bold'), text=')', bg='orange',
                         command=lambda: btn(')')
                         ).grid(row=6, column=3)
...
txtMeal['values'] = ('Fried Rice', 'Fried Rice & Chicken', 'Vegplate', 'Garlicbread', 'Burger','Dosa','Stufeed Paratha','Salad','ChikeenTikka','Panner+Naan','Spclplate')
...
txtQtyofMeal = Entry(f1, font=('arial', 16, 'bold'), textvariable=Meal1, bd=10, insertwidth=4, bg='white',
                     justify='right')
...
txtDrink['values'] = ('Bottled Water', 'Coca Cola', 'Pepsi', 'SoftDrink', 'Red Wine','Juice', 'Milk Shake','Choclateshake','Greentea','coffee')
...
txtQtyofDrink = Entry(f1, font=('arial', 16, 'bold'), textvariable=Drink1, bd=10, insertwidth=4, bg='white',
                      justify='right')
...
txtService = Entry(f1, font=('arial', 16, 'bold'), textvariable=Service_Charge, fg='white', bd=10, insertwidth=4,
                   bg='blue',
                   justify='right')
...
lblCunCon = Label(f1, font=('arial', 16, 'bold italic'),
